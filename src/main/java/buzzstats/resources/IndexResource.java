...
public class IndexResource {
  ...
  public IndexView getHomePage() {
    ...
    view.setMostDiscussed(
        thingsDao.findOrderByCommentsDesc(0, ROW_COUNT).stream()
            .map(ThingEntity::toThing)
            .collect(Collectors.toList()));
    view.setMostVoted(
        thingsDao.findOrderByScoreDesc(0, ROW_COUNT).stream()
            .map(ThingEntity::toThing)
            .collect(Collectors.toList()));
    view.setMostRecent(
        thingsDao.findOrderByPublishedAtDesc(0, ROW_COUNT).stream()
            .map(ThingEntity::toThing)
            .collect(Collectors.toList()));
    view.setOldestChecked(
        thingsDao.findOrderByLastCheckedAt(0, ROW_COUNT).stream()
            .map(ThingEntity::toThing)
            .collect(Collectors.toList()));
    view.setRecentlyChecked(
        thingsDao.findOrderByLastCheckedAtDesc(0, ROW_COUNT).stream()
            .map(ThingEntity::toThing)
            .collect(Collectors.toList()));
    view.setLastModified(
        thingsDao.findOrderByLastModifiedAtDesc(0, ROW_COUNT).stream()
            .map(ThingEntity::toThing)
            .collect(Collectors.toList()));
    return view;
  }
