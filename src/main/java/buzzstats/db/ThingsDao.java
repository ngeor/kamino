...
public interface ThingsDao {
    ...
    @RegisterBeanMapper(ThingEntity.class)
    List<ThingEntity> findOrderByCommentsDesc(@Bind("offset") int offset, @Bind("row_count") int rowCount);

    @SqlQuery("SELECT * FROM things ORDER BY score DESC LIMIT :offset, :row_count")
    @RegisterBeanMapper(ThingEntity.class)
    List<ThingEntity> findOrderByScoreDesc(@Bind("offset") int offset, @Bind("row_count") int rowCount);

    @SqlQuery("SELECT * FROM things ORDER BY published_at DESC LIMIT :offset, :row_count")
    @RegisterBeanMapper(ThingEntity.class)
    List<ThingEntity> findOrderByPublishedAtDesc(@Bind("offset") int offset, @Bind("row_count") int rowCount);

    @SqlQuery("SELECT * FROM things ORDER BY last_checked_at LIMIT :offset, :row_count")
    @RegisterBeanMapper(ThingEntity.class)
    List<ThingEntity> findOrderByLastCheckedAt(@Bind("offset") int offset, @Bind("row_count") int rowCount);

    @SqlQuery("SELECT * FROM things ORDER BY last_checked_at DESC LIMIT :offset, :row_count")
    @RegisterBeanMapper(ThingEntity.class)
    List<ThingEntity> findOrderByLastCheckedAtDesc(@Bind("offset") int offset, @Bind("row_count") int rowCount);

    @SqlQuery("SELECT * FROM things ORDER BY last_modified_at DESC LIMIT :offset, :row_count")
    @RegisterBeanMapper(ThingEntity.class)
    List<ThingEntity> findOrderByLastModifiedAtDesc(@Bind("offset") int offset, @Bind("row_count") int rowCount);
    ...
    @SqlUpdate(
        "INSERT INTO things "
            ...
            + "VALUES (:title, :url, :score, :username, :publishedAt, :comments, :internalUrl, :createdAt, :lastModifiedAt, :lastCheckedAt)"
    )
    @GetGeneratedKeys
    ...
    @SqlUpdate(
        "UPDATE things SET last_checked_at=:lastCheckedAt WHERE id=:id"
    )
    void update(
        @Bind("lastCheckedAt") LocalDateTime lastCheckedAt,
        @Bind("id") long id
