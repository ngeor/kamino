...
namespace BuzzStats.UnitTests.Crawl
{
    ...
    public class PollerTest
    {
        ...
        [Test]
        public void ShouldIgnoreStoriesAlreadyCollectedByOtherSources()
        {
            // arrange
            IStoryQuery storyQuery = StubStoryQuery.Mock()
                .SetupExcludeIds(new[] {123});

            ILeaf foundLeaf = Mock.Of<ILeaf>(l => l.StoryId == 123);
            ILeafProducerMonitor leafProducerMonitor = Mock.Of<ILeafProducerMonitor>(
                p => p.GetCollectedLeaves() == new[] {foundLeaf});
            Poller poller = Create(
                dbContext: storyQuery.BindDbContext(),
                leafProducerMonitor: leafProducerMonitor);
        ...
        [Test]
        public void ShouldIgnoreStoriesThatDidNotHaveChanges()
        {
            // arrange
        ...
        [Test]
        public void ShouldKeepIgnoringStoriesAfter2ProducerCycles()
        {
            // arrange
            StubMessageBus messageBus = new StubMessageBus();
            IStoryQuery storyQuery = StubStoryQuery.Mock()
